    ...
    try {
      ...
      switch (setKey) {
        ...
        case 'fromBeginningTxSet':
          let oldestTrans = state.transactions.map(a => Object.assign({}, a));
          if (oldestTrans.length > 0) oldestTrans = oldestTrans.reduce((a, b) => (moment(a.date) < moment(b.date)) ? a : b);
        ...
        case 'customTxSet':
          ...
          let en = moment(en, 'YYYY-MM-DD', true);
...
function buildTxArray(type) {
  ...
  self.onmessage = e => {
    ...
    let PromiseArray = buildTxArray(e.data.webWorkerType);
      Promise.all(PromiseArray).then(() => {
        self.postMessage({ type: 'setTxData', payload: state.txData});
        if (e.data.webWorkerType === 'initial') {
          self.postMessage({ type: 'doneLoading', payload: true });
          let SecondPromiseArray = buildTxArray('afterInitial');
          Promise.all(SecondPromiseArray).then(() => {
            self.postMessage({ type: 'setTxData', payload: state.txData});
            self.postMessage({ type: 'setReloading', payload: true });
          })
        } 
        else self.postMessage({ type: 'setReloading', payload: true });
      });
